        ...
        contents: [
          {
            ...
            parts: [
              ...
              {
                text: `Пожалуйста, проанализируй этот файл. 
Если это официальное письмо или документ, ВМЕСТО обычного текста выведи ТОЛЬКО специальный тег <draftcard> с JSON внутри, содержащим следующие поля:
- number: номер документа (если есть, иначе пустая строка)
- org: от кого (организация)
- date: дата документа (YYYY-MM-DD)
- summary: краткая выжимка (1-2 предложения)
- priority: число от 0 до 100 (где 100 - супер срочно, 50 - обычно)
- deadlineDate: рассчитай дедлайн в формате YYYY-MM-DD (например, если жалоба/срочно - +3-15 дней, если обычный запрос - +30 дней от даты документа).
...
<draftcard>
{
  ...
  "org": "ООО Ромашка",
  "date": "2026-05-01",
  "summary": "Просьба о выделении бюджета на 3 квартал",
  "priority": 70,
  "deadlineDate": "2026-05-15"
}
</draftcard>

Если это не документ, а просто картинка, просто опиши её словами на русском языке. Никаких <draftcard> тегов не выводи.`,
              },
            ],
          },
        ],
